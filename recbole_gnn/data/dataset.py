...
class GeneralGraphDataset(RecBoleDataset):
    ...
    def get_norm_adj_mat(self, enable_sparse=False):
        ...
        if enable_sparse:
            if not is_sparse:
                ...
                adj_t = self.edge_index_to_adj_t(
    ...
    def get_bipartite_inter_mat(self, row="user", row_norm=True):
        ...
        if row_norm:
            ...
            row_norm_deg = 1.0 / torch.sqrt(
                torch.where(row_deg == 0, torch.ones([1]), row_deg)
            )
            col_norm_deg = 1.0 / torch.sqrt(
                torch.where(col_deg == 0, torch.ones([1]), col_deg)
            )
...
class LESSRDataset(SessionGraphDataset):
    def session_graph_construction(self):
        ...
        for i, seq in enumerate(tqdm(list(torch.chunk(item_seq, item_seq.shape[0])))):
            ...
            shortcut_edge = (
                torch.cat(sub_edges, dim=-1).unique(dim=-1)
                if len(sub_edges) > 0
                else empty_edge
            )
...
class GCEGNNDataset(SequentialDataset):
    ...
    def reverse_session(self):
        ...
        for i in tqdm(range(item_seq.shape[0])):
            item_seq[i, : item_seq_len[i]] = item_seq[i, : item_seq_len[i]].flip(
                dims=[0]
            )
    ...
    def bidirectional_edge(self, edge_index):
        ...
        idc = ed.unsqueeze(1).expand(-1, seq_len, 2) == ed2.unsqueeze(0).expand(
            seq_len, -1, 2
        )
    ...
    def session_graph_construction(self):
        ...
        for i, seq in enumerate(tqdm(list(torch.chunk(item_seq, item_seq.shape[0])))):
            ...
            edge_attr_backward = torch.where(
                self.bidirectional_edge(edge_index_backward), 3, 1
            )
            edge_backward = torch.cat(
                [edge_index_backward, edge_attr_backward.unsqueeze(0)], dim=0
            )

            edge_index_forward = torch.stack([alias_seq[1:], alias_seq[:-1]])
            edge_attr_forward = torch.where(
                self.bidirectional_edge(edge_index_forward), 3, 2
            )
            edge_forward = torch.cat(
                [edge_index_forward, edge_attr_forward.unsqueeze(0)], dim=0
            )

            edge_index_selfloop = torch.stack([alias_seq, alias_seq])
            edge_selfloop = torch.cat(
                [edge_index_selfloop, torch.zeros([1, edge_index_selfloop.shape[1]])],
                dim=0,
            )

            edge = torch.cat(
                [edge_backward, edge_forward, edge_selfloop], dim=-1
            ).long()
...
class SocialDataset(GeneralGraphDataset):
    ...
    def _get_field_from_config(self):
        ...
        self.logger.debug(
            set_color("net_src_field", "blue") + f": {self.net_src_field}"
        )
        self.logger.debug(
            set_color("net_tgt_field", "blue") + f": {self.net_tgt_field}"
        )
    ...
    def _filter_net_by_inter(self):
        ...
        self.net_feat.drop(
            self.net_feat.index[~self.net_feat[self.net_src_field].isin(inter_uids)],
            inplace=True,
        )
        self.net_feat.drop(
            self.net_feat.index[~self.net_feat[self.net_tgt_field].isin(inter_uids)],
            inplace=True,
        )
    ...
    def __str__(self):
        info = [
            super().__str__(),
            set_color("The number of social network relations", "blue")
            + f": {self.net_num}",
        ]  # yapf: disable
        return "\n".join(info)
    ...
    def _load_net(self, token, dataset_path):
        self.logger.debug(
            set_color(f"Loading social network from [{dataset_path}].", "green")
        )
    ...
    def _init_alias(self):
        ...
        self._set_alias(
            "user_id", [self.uid_field, self.net_src_field, self.net_tgt_field]
        )
    ...
    def net_matrix(self, form="coo", value_field=None):
        ...
        return self._create_sparse_matrix(
            self.net_feat, self.net_src_field, self.net_tgt_field, form, value_field
        )
