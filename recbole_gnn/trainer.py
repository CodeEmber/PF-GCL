...
class PFGCL12Trainer(Trainer):

    def __init__(self, config, model):
        super(PFGCL12Trainer, self).__init__(config, model)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            self.optimizer.step()
            if self.gpu_available and show_progress:
                iter_data.set_description_str(
                    set_color(f"Train {epoch_idx:>5}", "pink")
                )
...
class HMLETTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        if epoch_idx > self.warm_up_epochs:
            # Temp decay
            gum_temp = self.ori_temp * math.exp(
                -self.gum_temp_decay * (epoch_idx - self.warm_up_epochs)
            )
            self.model.gum_temp = max(gum_temp, self.min_temp)
            self.logger.info(
                f"Current gumbel softmax temperature: {self.model.gum_temp}"
            )
